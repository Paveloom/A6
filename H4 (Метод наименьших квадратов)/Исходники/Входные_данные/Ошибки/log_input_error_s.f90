...
implicit none
     ...
     module procedure log_input_error
          ...
          select case (error_code) ! Проверка кода ошибки
               ...
               case ('WD_file') ! Встречается в deallocate_input_s
               ...
               case ('NA_file') ! Встречается в deallocate_input_s

                    write(*,'(/, 5x, a, /, 5x, a, /)') 'log_input_error:', 'Память под переменную file объекта&
                                                                           & типа input_type не была выделена.'
                    stop
