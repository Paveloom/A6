...
implicit none
     ...
     module procedure log_result_error
          ...
          select case (error_code) ! Проверка кода ошибки
               ...
               case ('NA_evalues') ! Встречается в deallocate_result_s

                    write(*,'(/, 5x, a, /, 5x, a, /)') 'log_result_error:', 'Память под матрицу evalues объекта типа&
                                                                           & result_type не была выделена. Невозможно& 
                                                                           & освободить память.'
                    stop
